...
function updateSummary() {
  ...
  document.getElementById("summaryStrength").textContent = strength.value
  ...
  document.getElementById("summaryPack").textContent = packSize.value || "—";

  document.getElementById("summaryPrice").textContent = sellingPrice.value
    ? `₹${sellingPrice.value}`
    : "—";
  ...
  if (margin < 0) {
    ...
    profitMargin.style.background = "var(--primary-light)";

    profitMargin.style.color = "var(--primary)";

    summaryProfit.style.color = "var(--text-main)";
  }
}
...
function setupDocumentUpload(inputId, fileNameId, removeBtnId, storageField) {
...
saveBtn.addEventListener("click", function (e) {
  ...
  for (const field of requiredFields) {
    if (!field.input.value.trim()) {
      ...
      showCustomAlert("Required Field", field.message);
  ...
  const packSizeValue = packSize.value.trim();
  const quantityFromPack = parseInt(packSizeValue, 10);

  const productData = {
    id: Date.now(),
    name: medicineName.value.trim(),
    category: dosageForm.value.trim(),
    quantity: Number.isFinite(quantityFromPack) ? quantityFromPack : 1,
    ...
    details: {
      ...
      sellingPrice: parseFloat(sellingPrice.value) || 0,
      mrpPrice: parseFloat(document.getElementById("mrpPrice").value) || 0,
      taxRate: taxRate.value,
      profitMargin: profitMargin.textContent,
      description: document.querySelectorAll(".form-textarea")[0].value,
      indications: document.querySelectorAll(".form-textarea")[1].value,
      dosageInstructions: document.querySelectorAll(".form-textarea")[2].value,
      sideEffects: document.querySelectorAll(".form-textarea")[3].value,
      contraindications: document.querySelectorAll(".form-textarea")[4].value,
      productImage: imageInput.files[0] ? imageInput.files[0].name : "",
  ...
  // Success Alert
  showCustomAlert("Success!", "Product saved successfully.");
